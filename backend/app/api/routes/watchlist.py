...
@router.get(
    ...
    summary="查询我的自选股票",
    description="按 X-User-Key 返回当前小程序用户已经加入自选的股票；数据持久化在数据库中。",
    response_description="自选股票列表",
)
def get_watchlist(repository: DatabaseMarketRepository = Depends(get_market_repository)) -> WatchlistResponse:
...
@router.post(
    ...
    summary="加入自选股票",
    description="把指定六位股票代码加入当前用户自选。若股票尚未进入异动候选池，后端会按代码同步真实行情后再保存。",
